...
class Core(ActionBase):
    ...
    def get_config_rows(self) -> list:
        self.add_new_ip_entry = Adw.EntryRow(title=self.plugin_base.locale_manager.get("actions.add_ip_entry.title"), input_purpose=Gtk.InputPurpose.FREE_FORM, show_apply_button=True)
        self.add_new_ip_entry.connect("apply", self.on_ip_address_added)
        ...
        self.device_list = Adw.ComboRow(title=self.plugin_base.locale_manager.get("actions.device_list.title"), model=self.registered_devices)
        ...
        return [self.add_new_ip_entry, self.device_list, self.connection_banner]
    ...
    def load_default_config(self):
        ...
        if saved_ip_address:
            ...
            self.plugin_base.backend.register_new_device(saved_ip_address, saved_ip_address)
    ...
    def on_ip_address_added(self, entry: Adw.EntryRow):
        new_ip_address = entry.get_text()
    ...
    def set_ip_address(self, entry: Adw.EntryRow, *args):
        ...
        ip_by_index = list(self.plugin_base.backend.get_devices().values())[entry.get_selected()].ip_address

        print("Save new selected ip address ", ip_by_index)
